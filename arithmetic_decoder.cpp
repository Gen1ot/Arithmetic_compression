...
void decoder(const char* input_name = "encoded.txt", const char* output_name = "output.txt")
{
    ...
    for (int i = 0; i < col_letters; i++)
    {
        ...
        if (!feof(input_file))
        {
            fread(reinterpret_cast<char*>(&alfabet[character]), sizeof(unsigned short), 1, input_file);
        }
        else
        {
    ...
    unsigned short* table = new unsigned short[arr.size() + 2];
    table[0] = 0;
    table[1] = 1;
    ...
    unsigned int second_qtr = first_qtr * 2;
    ...
    {   // Read from input file
        ...
        for (;;) {
            if (high_v < second_qtr) {}
            else if (low_v >= second_qtr)
            {
                low_v -= second_qtr;
                high_v -= second_qtr;
                value -= second_qtr;
